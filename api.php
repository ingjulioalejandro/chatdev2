...
}elseif(isset($DATA_OBJ->data_type) && $DATA_OBJ->data_type == "filessharing")
{
	//login
	include("includes/filesSharing.php");
}elseif(isset($DATA_OBJ->data_type) && $DATA_OBJ->data_type == "filessharingUpload")
{
	//login
	include("includes/filesSharing_Upload.php");
...
function message_left($data,$row)
{
	...
	<img id='trash' src='ui/icons/ntrash.png' onclick='delete_message(event)' msgid='$data->id' />
	</div> ";
...
function message_controls()
{
	...
	<div style='display:flex;width:100%;height:30px;'>
		<label for='message_file'><img src='ui/icons/nclip.png' style='opacity:0.8;width:20px;margin:5px;cursor:pointer;' ></label>
		...
		<input style='flex:1;cursor:pointer;background-color: #27344b;color: white; border: solid 2px white;border-radius: 50px' type='button' value='send' onclick='send_message(event)'/>
	</div>
	</div>";
}
